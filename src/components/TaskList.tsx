...
export function TaskList({ name, status, className }: TaskListProps) {
    ...
    const { ref } = useDroppable({ id: status.toString() });
...
function TaskListItem({ task }: TaskListItemProps) {
    ...
    return (
        <div
            ref={ref}
            className="flex flex-col gap-1 rounded-lg bg-blue-900/20 border border-blue-900/40 px-4 py-3 shadow-sm hover:border-blue-800/60 transition-colors">
